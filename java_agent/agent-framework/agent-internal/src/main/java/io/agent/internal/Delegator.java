...
import io.agent.annotations.Returned;
import io.agent.annotations.Thrown;
...
public class Delegator {
    ...
    private static List<Class<?>> stripReturnedAndThrown(Method hookMethod) {
        ...
        if (allTypes.length != annotations.length) {
            throw new HookException("Method.getParameterAnnotations() returned an unexpected value. This is a bug in agent.");
        }
    ...
    private boolean shouldBeSkipped(Class<?> hookClass) {
        return hookClass.getAnnotation(io.agent.annotations.Hook.class).skipNestedCalls()
