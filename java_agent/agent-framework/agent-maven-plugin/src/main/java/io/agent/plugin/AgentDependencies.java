...
class AgentDependencies {
    ...
    static AgentDependencies init(PluginDescriptor pluginDescriptor) throws MojoExecutionException {
        ...
        List<ExpectedDependency> expectedDependencies = Arrays.asList(
                new ExpectedDependency(pluginGroupId, "agent-agent"),
                new ExpectedDependency(pluginGroupId, "agent-internal"),
                new ExpectedDependency(pluginGroupId, "agent-api"),
    ...
    /**
     * Artifact for the agent-agent module.
     */
    ...
    /**
     * Artifact for all other runtime dependencies except agent-agent.
     */
    ...
    void assertNoConflict(Artifact artifact) throws MojoExecutionException {
        ...
        if (builtInVersion.isPresent() && ! builtInVersion.get().equals(artifact.getVersion())) {
            ...
            throw new MojoExecutionException("Conflicting dependencies: Your project includes " + artifactName +
                    " version " + artifact.getVersion() + " but the agent-maven-plugin is built with version " + builtInVersion.get());
        }
    }
    ...
    private Predicate<Artifact> isAgent() {
        return artifact -> artifact.getGroupId().equals(pluginGroupId) && artifact.getArtifactId().equals("agent-agent");
    }
